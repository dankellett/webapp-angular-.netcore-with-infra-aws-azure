...
resource "aws_security_group" "allow_sqlserver_port" {
  name        = "allow_sqlServer_port"
  description = "Allow sqlServer inbound traffic"
  vpc_id      = module.vpc.vpc_id
...
module "db" {
  ...
  vpc_security_group_ids = [module.vpc.vpc_default_security_group_id, aws_security_group.allow_sqlserver_port.id]
